...
ALTER TABLE health_conversations ADD COLUMN IF NOT EXISTS summary text;
ALTER TABLE health_conversations ADD COLUMN IF NOT EXISTS message_count integer DEFAULT 0;
...
CREATE TABLE IF NOT EXISTS health_user_files (
  id uuid DEFAULT uuid_generate_v4() PRIMARY KEY,
  user_id uuid REFERENCES auth.users ON DELETE CASCADE NOT NULL,
...
DROP POLICY IF EXISTS "Users can manage own health files" ON health_user_files;
CREATE POLICY "Users can manage own health files"
  ON health_user_files FOR ALL
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS health_user_files_user_idx ON health_user_files(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS health_user_files_status_idx ON health_user_files(status);
